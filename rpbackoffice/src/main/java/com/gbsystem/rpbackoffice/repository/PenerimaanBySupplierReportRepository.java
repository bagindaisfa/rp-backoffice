...
public interface PenerimaanBySupplierReportRepository extends JpaRepository<PenerimaanBySupplierReport, Long> {
	@Query(value = "select p.tanggal_penerimaan, p.penerimaan_code, p.id_supplier, p.nama_supplier, "
			+ "p.artikel, m.kategori, m.tipe, p.nama_barang, p.kuantitas, p.ukuran, p.hpp, p.harga_jual from penerimaan_supplier p "
