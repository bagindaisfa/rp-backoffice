...
public interface PenyimpananBarangMasukReportRepository extends JpaRepository<PenyimpananBarangMasukReport, Long> {
	@Query(value = "select p.id, p.tanggal_masuk AS tanggal_masuk, date_format(:tanggal_transaksi,'%Y-%m-%d') AS tanggal_transaksi, "
			+ "p.penerimaan_code, p.artikel AS artikel, m.nama_kategori AS kategori, "
			+ "m.type_name AS tipe, p.nama_barang AS nama_barang, p.ukuran AS ukuran, "
			+ "p.kuantitas AS kuantitas, p.hpp AS hpp, p.harga_jual AS harga_jual from penyimpanan_masuk p "
			+ "left join master_product m on m.artikel_product = p.artikel AND m.ukuran=p.ukuran AND m.rowstatus = 1 "
