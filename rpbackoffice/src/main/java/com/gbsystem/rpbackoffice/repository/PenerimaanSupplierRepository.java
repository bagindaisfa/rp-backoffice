...
List<PenerimaanSupplier> findByRowstatus(@Param("rowstatus") int rowstatus);
	...
	@Query(value = "SELECT * FROM penerimaan_supplier WHERE rowstatus = 1 AND (penerimaan_code LIKE %:keyword% OR pembelian_code LIKE %:keyword% OR nama_supplier LIKE %:keyword%)", nativeQuery = true)
