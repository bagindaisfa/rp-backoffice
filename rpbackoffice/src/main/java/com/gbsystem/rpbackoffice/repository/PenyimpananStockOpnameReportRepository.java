...
public interface PenyimpananStockOpnameReportRepository extends JpaRepository<PenyimpananStockOpnameReport, Long> {
	@Query(value = "select id, tanggal_so, date_format(:tanggal_transaksi,'%Y-%m-%d') AS tanggal_transaksi, "
			+ "artikel, kategori, tipe, nama_barang, kuantitas_masuk, kuantitas_keluar, stock, "
			+ "stock_opname, keterangan from stock_opname where rowstatus=1 and "
			+ "date_format(tanggal_so,'%Y-%m-%d') != :tanggal_transaksi", nativeQuery = true)
