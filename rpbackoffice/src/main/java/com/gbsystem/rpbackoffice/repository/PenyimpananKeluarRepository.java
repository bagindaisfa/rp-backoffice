...
public interface PenyimpananKeluarRepository extends JpaRepository<PenyimpananKeluar, Long> {
	...
	@Query(value = "SELECT * FROM penyimpanan_keluar WHERE rowstatus = 1 AND pengiriman_code= :pengiriman_code AND artikel=:artikel", nativeQuery = true)
	PenyimpananKeluar getPenyimpananByPengirimanCodeandArtikel(String pengiriman_code, String artikel);
	...
	Double generateKuantitasKeluar(String artikel, Date tanggal_awal, Date tanggal_akhir);
}
